...
class Capsule extends Shape {
  ...
  num computeMomentOfInertia(num mass) {
    ...
    num r = this.radius;
    // Approximate with rectangle
    num // 2*r is too much, 0 is too little
    h = r * 2;
    // Approximate with rectangle
    num w = this.length + r;
  ...
  computeAABB(AABB out, [vec2 position, num angle]) {
    ...
    // Get bounds
    vec2.set(out.upperBound, max(r.x + radius, -r.x + radius), max(r.y + radius, -r.y + radius));
    vec2.set(out.lowerBound, min(r.x - radius, -r.x - radius), min(r.y - radius, -r.y - radius));
